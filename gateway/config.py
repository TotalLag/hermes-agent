...
def _apply_env_overrides(config: GatewayConfig) -> None:
    ...
    if matrix_token or os.getenv("MATRIX_PASSWORD"):
        ...
        matrix_require_mention = os.getenv("MATRIX_REQUIRE_MENTION", "").lower() in (
            "true",
            "1",
            "yes",
        )
        config.platforms[Platform.MATRIX].extra["require_mention"] = (
            matrix_require_mention
        )
        matrix_free_response = os.getenv("MATRIX_FREE_RESPONSE_ROOMS", "")
        if matrix_free_response:
            config.platforms[Platform.MATRIX].extra["free_response_rooms"] = [
                r.strip() for r in matrix_free_response.split(",") if r.strip()
            ]
    ...
    hiclaw_mc_host = os.getenv("HICLAW_MC_HOST")
    if hiclaw_mc_host:
        os.environ["HICLAW_MC_HOST"] = hiclaw_mc_host

    hiclaw_bucket = os.getenv("HICLAW_BUCKET")
    if hiclaw_bucket:
        os.environ["HICLAW_BUCKET"] = hiclaw_bucket

    hiclaw_access_key = os.getenv("HICLAW_ACCESS_KEY")
    if hiclaw_access_key:
        os.environ["HICLAW_ACCESS_KEY"] = hiclaw_access_key

    hiclaw_secret_key = os.getenv("HICLAW_SECRET_KEY")
    if hiclaw_secret_key:
        os.environ["HICLAW_SECRET_KEY"] = hiclaw_secret_key

    hiclaw_task_specs_prefix = os.getenv("HICLAW_TASK_SPECS_PREFIX")
    if hiclaw_task_specs_prefix:
        os.environ["HICLAW_TASK_SPECS_PREFIX"] = hiclaw_task_specs_prefix

    hiclaw_task_results_prefix = os.getenv("HICLAW_TASK_RESULTS_PREFIX")
    if hiclaw_task_results_prefix:
        os.environ["HICLAW_TASK_RESULTS_PREFIX"] = hiclaw_task_results_prefix
