...
def generate_manager_config(
    ...
    vals = {
        "HICLAW_MANAGER_GATEWAY_KEY": secrets["HICLAW_MANAGER_GATEWAY_KEY"],
        "HICLAW_MANAGER_PASSWORD": secrets["HICLAW_MANAGER_PASSWORD"],
        "HICLAW_WORKSPACE_DIR": _env("HICLAW_WORKSPACE_DIR", "/root/manager-workspace"),
        "HICLAW_MATRIX_DOMAIN": _env(
            "HICLAW_MATRIX_DOMAIN", "matrix-local.hiclaw.io:18080"
        ),
        "HICLAW_DOCKER_PROXY": _env(
            "HICLAW_DOCKER_PROXY", "http://hiclaw-docker-proxy:2375"
        ),
        "HICLAW_MINIO_BUCKET": _env("HICLAW_MINIO_BUCKET", "hiclaw"),
        "HICLAW_TASK_SPECS_PREFIX": _env("HICLAW_TASK_SPECS_PREFIX", "task-specs/"),
        "HICLAW_TASK_RESULTS_PREFIX": _env(
            "HICLAW_TASK_RESULTS_PREFIX", "task-results/"
        ),
        "HICLAW_MINIO_USER": _env("HICLAW_MINIO_USER", "root"),
        "HICLAW_MINIO_PASSWORD": _env("HICLAW_MINIO_PASSWORD", ""),
        "HICLAW_LLM_PROVIDER": _env("HICLAW_LLM_PROVIDER", "openai-compat"),
        "HICLAW_OPENAI_BASE_URL": _env("HICLAW_OPENAI_BASE_URL", ""),
        "HICLAW_DEFAULT_MODEL": _env("HICLAW_DEFAULT_MODEL", "MiniMax-M2.7"),
        "HICLAW_LLM_API_KEY": _env("HICLAW_LLM_API_KEY", ""),
        "MANAGER_TOKEN": manager_token,
    }

    config_text = f"""# Auto-generated by hermes-agent hiclaw Manager
version: "1"

hiclaw:
  manager:
    gateway_key: "{vals["HICLAW_MANAGER_GATEWAY_KEY"]}"
    password: "{vals["HICLAW_MANAGER_PASSWORD"]}"
    workspace: "{vals["HICLAW_WORKSPACE_DIR"]}"

  infrastructure:
    matrix_domain: "{vals["HICLAW_MATRIX_DOMAIN"]}"
    docker_proxy: "{vals["HICLAW_DOCKER_PROXY"]}"
    minio_bucket: "{vals["HICLAW_MINIO_BUCKET"]}"
    minio_prefix_tasks: "{vals["HICLAW_TASK_SPECS_PREFIX"]}"
    minio_prefix_results: "{vals["HICLAW_TASK_RESULTS_PREFIX"]}"
    minio_access_key: "{vals["HICLAW_MINIO_USER"]}"
    minio_secret_key: "{vals["HICLAW_MINIO_PASSWORD"]}"

  llm:
    provider: "{vals["HICLAW_LLM_PROVIDER"]}"
    base_url: "{vals["HICLAW_OPENAI_BASE_URL"]}"
    model: "{vals["HICLAW_DEFAULT_MODEL"]}"
    api_key: "{vals["HICLAW_LLM_API_KEY"]}"

platforms:
  matrix:
    homeserver: "http://{vals["HICLAW_MATRIX_DOMAIN"]}"
    access_token: "{vals["MANAGER_TOKEN"]}"
