...
def _hiclaw_manager_enabled() -> bool:
    if os.getenv("HICLAW_MANAGER_ENABLED", "").lower() in ("1", "true", "yes"):
        return True
    try:
        import yaml as _y
        ...
        if _cfg_path.exists():
            ...
            if _cfg.get("hiclaw", {}).get("manager", {}).get("enabled"):
                return True
    except Exception:
        pass
    return False
...
class GatewayRunner:
    ...
    def __init__(self, config: Optional[GatewayConfig] = None):
        ...
        self._hiclaw_manager = None
        self._hiclaw_manager_task = None
        if _hiclaw_manager_enabled():
            try:
                from gateway.hiclaw.manager_service import HiclawManager

                self._hiclaw_manager = HiclawManager()
                logger.info("HiclawManager configured and ready to start")
            except Exception as e:
    ...
    async def start(self) -> bool:
        ...
        if self._hiclaw_manager is not None:
            self._hiclaw_manager_task = asyncio.create_task(
    ...
    async def _handle_model_command(self, event: MessageEvent) -> str:
        ...
        if not args:
            ...
            lines.append("To change: `/model model-name`")
            lines.append(
                "Switch provider: `/model provider-name` or `/model provider:model-name`"
            )
            return "\n".join(lines)
