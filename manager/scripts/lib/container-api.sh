...
# Get the Manager container's Docker network name (for Workers to join)
container_get_manager_network() {
    local container_id=$(hostname)
    local resp
    if [ -n "${HICLAW_CONTAINER_API}" ]; then
        resp=$(curl -s "${HICLAW_CONTAINER_API}/containers/${container_id}/json" 2>/dev/null)
    else
        resp=$(curl -s --unix-socket "${CONTAINER_SOCKET}" "http://localhost/containers/${container_id}/json" 2>/dev/null)
    fi
    echo "${resp}" | python3 -c "import sys,json; d=json.load(sys.stdin); print(list(d[\"NetworkSettings\"][\"Networks\"]).keys())[0])" 2>/dev/null
}
...
container_create_worker() {
    ...
    local manager_network
    manager_network=$(container_get_manager_network)
    if [ -z "${manager_network}" ]; then
        _log "WARNING: Cannot determine Manager network; worker will use default bridge"
    fi
    ...
    # Build NetworkingConfig to join the Manager's Docker network
    local networking_config=""
    if [ -n "${manager_network}" ]; then
        networking_config="{\"${manager_network}\": {}}"
        _log "  Network: ${manager_network}"
    fi
    ...
    create_payload=$(cat <<PAYLOAD
{
    ...
    "HostConfig": ${host_config}${networking_config:+, \"NetworkingConfig\": ${networking_config}}
}
PAYLOAD
)
